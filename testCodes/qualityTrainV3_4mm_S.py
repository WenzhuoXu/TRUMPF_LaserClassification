...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, N_epochs + 1):
        ...
        for batch in training_dataloader:
            ...
            loss_train.backward()
            optimizer.step()
        print("prediction speed: {:.4f}".format(
            output['speed'][1],
            )
        )

        print("target speed: {:.4f}".format(
            target_labels['speed'][1],
            )
        )

        print("epoch {:4d}, loss: {:.4f}, speed: {:.4f}".format(
            epoch,
