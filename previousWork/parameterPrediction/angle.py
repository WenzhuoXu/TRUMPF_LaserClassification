...
# set the dataloader
train_root = '/content/angle_train.zip_files/train'
val_root = '/content/angle_train.zip_files/val'
...
val_loader = torch.utils.data.DataLoader(val_datasets, batch_size=batch_size, shuffle=False, num_workers=8)
...
model_weight_path = '/content/resnext50_32x4d.pth'
...
def val(model, loader, device, data_size, criterion):
    ...
    with torch.no_grad():
        ...
        print('val epoch loss:{:.2f}, accuracy:{:.2f}% [{:d}/{:d}]'.format(test_loss, accuracy, correct, data_size))
...
for epoch in range(EPOCH):
    ...
    model = train(EPOCH, model, train_loader, device, optimizer, criterion)
    acc = val(model, val_loader, device, len(val_datasets), criterion)
    if acc > best_acc:
...
torch.save(model.state_dict(), '/content/test_angle.pth')
